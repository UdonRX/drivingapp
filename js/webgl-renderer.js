...
class MeshBuilder{
  ...
  reset(){this.count=0}
  vertex(x,y,z,c){const i=this.count*6,d=this.data;if(i+5>=d.length)return;d[i]=x;d[i+1]=y;d[i+2]=z;d[i+3]=c[0];d[i+4]=c[1];d[i+5]=c[2];this.count++}
  tri(a,b,c,col){this.vertex(a.x,a.y,a.z,col);this.vertex(b.x,b.y,b.z,col);this.vertex(c.x,c.y,c.z,col)}
...
let gl,program,roadBuffer,sceneryBuffer,rainBuffer,lastSceneryKey='',quality=1,canvasRef;
...
function uploadAndDraw(buffer,builder,usage=gl.DYNAMIC_DRAW){if(!builder.count)return;gl.bindBuffer(gl.ARRAY_BUFFER,buffer);gl.bufferData(gl.ARRAY_BUFFER,builder.data.subarray(0,builder.count*6),usage);setupBuffer(buffer);gl.drawArrays(gl.TRIANGLES,0,builder.count)}
...
function pushBox(b,x,y,z,sx,sy,sz,col){
  ...
  const a=point(x0,y0,z0),c=point(x1,y0,z0),d=point(x1,y1,z0),e=point(x0,y1,z0),f=point(x0,y0,z1),g=point(x1,y0,z1),h=point(x1,y1,z1),i=point(x0,y1,z1);
  b.quad(a,c,d,e,mulColor(col,.86));b.quad(g,f,i,h,col);b.quad(f,a,e,i,mulColor(col,.74));b.quad(c,g,h,d,mulColor(col,.94));b.quad(e,d,h,i,mulColor(col,1.08));b.quad(f,g,c,a,mulColor(col,.66));
}
function pushPyramid(b,x,y,z,r,h,col){
  const top=point(x,y+h,z),a=point(x-r,y,z-r),c=point(x+r,y,z-r),d=point(x+r,y,z+r),e=point(x-r,y,z+r);
  b.tri(a,c,top,mulColor(col,.88));b.tri(c,d,top,col);b.tri(d,e,top,mulColor(col,.78));b.tri(e,a,top,mulColor(col,.92));b.quad(a,e,d,c,mulColor(col,.58));
}
...
function buildGuardrails(){
  ...
  for(const side of[-1,1])for(let z=start;z<end;z+=16){
    ...
    pushBox(b,a.x,a.y,a.z,.10,.86,.10,mulColor(col,.75));
...
function buildScenery(){
  sceneryBuilder.reset();buildGuardrails();buildTunnelAndBridge();
...
function buildRain(){
  ...
  for(let i=0;i<count;i++){const phase=(state.worldTime*(18+hash1(i)*18)+hash1(i*4.7)*90)%90;const depth=5+phase,lateral=(hash1(i*7.1)-.5)*32,up=(hash1(i*3.9)*10)-2;const x=c.x+forwardX*depth+rightX*lateral,z=c.z+forwardZ*depth+rightZ*lateral,y=c.y+up;const len=.45+hash1(i*9.1)*.8;
    const a=point(x,y,z),d=point(x-rightX*.018,y-len,z-rightZ*.018),e=point(x+rightX*.018,y-len,z+rightZ*.018);rainBuilder.tri(a,d,e,col);
  }
}
...
export function renderWorld(){
  if(!gl)return;const pal=palettes[state.time][state.weather],clear=mixColor(rgb(pal[0]),rgb(pal[1]),.38);gl.clearColor(clear[0],clear[1],clear[2],1);gl.clear(gl.COLOR_BUFFER_BIT|gl.DEPTH_BUFFER_BIT);setUniforms();buildRoad();const key=sceneKey();if(key!==lastSceneryKey){buildScenery();lastSceneryKey=key}buildRain();uploadAndDraw(sceneryBuffer,sceneryBuilder,gl.DYNAMIC_DRAW);uploadAndDraw(roadBuffer,roadBuilder,gl.DYNAMIC_DRAW);uploadAndDraw(rainBuffer,rainBuilder,gl.DYNAMIC_DRAW);
}
