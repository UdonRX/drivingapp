...
function update(dt){
  ...
  state.worldTime+=dt;updateVehicle(dt);updateCamera(dt);updateEnvironment();updateAudio(dt);updateHud();updatePerformance(dt);
}
function frame(t){
  const dt=Math.min(.045,Math.max(.001,(t-state.lastT)/1000));state.lastT=t;update(dt);renderWorld();requestAnimationFrame(frame)
}
